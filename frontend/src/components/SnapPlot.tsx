...
import { dbOffset, SNAP_FREQ_LIMITS, SNAP_REFERENCE_POWER } from './snapPower';
...
export function SnapPlot({freq, values, reference, range, trend, powerReference=SNAP_REFERENCE_POWER}:{freq:number[];values:Value[]|null;reference?:Value[]|null;range:[number,number];trend?:SnapTrend;powerReference?:number}) {
  ...
  const t0=trend?.start??SNAP_FREQ_LIMITS[0],t1=trend?.end??SNAP_FREQ_LIMITS[1];
  ...
  const shift=dbOffset(powerReference), display=(v:Value)=>v===null?null:v+shift;
  const axis=trend?trend.points.map(p=>p.ts):freq, line=(trend?trend.points.map(p=>p.power_db):values??[]).map(display);
  ...
  const ticks=trend?Array.from({length:tickCount},(_,i)=>t0+(t1-t0)*i/(tickCount-1)):niceTicks(t0,t1,Math.min(14,(right-left)/42));
  const date=(t:number)=>new Date(t*1000).toISOString().slice(5,16).replace('T',' ');
  return <div className="snap-plot" ref={host}>
    <svg viewBox={`0 0 ${width} ${height}`} role="img" data-power-reference={powerReference} data-x-min={t0} data-x-max={t1} data-y-min={lo} data-y-max={hi} aria-label={trend?'Full-band power (dB) versus time (UTC)':'Power (dB) versus frequency (MHz), 4096 channels'}
      ...
      <text transform={`translate(14 ${(top+bottom)/2}) rotate(-90)`} textAnchor="middle">{powerReference===1?'Power (dB re 1 native)':'Power (dB re 10⁻¹⁰ native)'}</text>
      <g clipPath={`url(#${clip})`}>
        {!trend&&reference&&<path className="snap-reference" d={trace(freq,reference.map(display))}/>}
        <path className="snap-trace" d={trace(axis,line,trend?.points.map(p=>p.eq_epoch))}/>
        {trend&&trend.points.map(p=>p.power_db===null?null:<circle key={p.ts} cx={x(p.ts)} cy={y(p.power_db+shift)} r="3" fill="#194fa3"><title>{date(p.ts)} UTC · {(p.power_db+shift).toFixed(2)} dB · EQ {p.eq_epoch??'unknown'}</title></circle>)}
      </g>
