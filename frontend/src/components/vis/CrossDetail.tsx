...
import type { VisQuantity, VisRef, VisSet, VisUnits, VisWaterfallResponse } from "../../lib/types";
...
export default function CrossDetail({ i, j, antI, antJ, set, quantity, units, reference, useMock, onBack }: CrossDetailProps) {
  ...
  useEffect(() => {
    let cancelled = false;
    const p = useMock
      ? mockGetVisSpectra("latest", set, "cross", quantity, units, reference)
      : getVisSpectra({ ts: "latest", set, pairs: "cross", quantity, units, ref: reference });
    p.then((r) => {
      if (cancelled) return;
  ...
  }, [i, j, set, quantity, units, reference, useMock]);
  ...
  useEffect(() => {
    ...
    const p = useMock
      ? mockGetVisWaterfall(i, j, t0, t1, quantity, units, reference)
      : getVisWaterfall({ i, j, t0, t1, quantity, units, ref: reference });
    p.then((r) => !cancelled && setWaterfall(r)).catch(() => undefined);
    return () => {
      cancelled = true;
    };
  }, [i, j, range, customFrom, quantity, units, reference, useMock]);
    ...
    return [
      {
        ...
        colorbar: {
          title: { text: units, font: { size: 11, color: MUTED, family: SANS } },
          thickness: 10,
          outlinewidth: 0,
  ...
  return (
    <div>
      ...
      {quantity === "phase" && <p className="detail__caption">{SAWTOOTH_NOTE}</p>}
