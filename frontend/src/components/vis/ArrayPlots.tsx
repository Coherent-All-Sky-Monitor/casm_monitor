...
export function clock(t:number,zone:string) {return new Intl.DateTimeFormat("en-US",{timeZone:zone,hour:"2-digit",minute:"2-digit",hour12:false}).format(new Date(t*1000));}
...
export function DynamicSpectrum({tile,data,zone,quantity}:{tile:Tile;data:Snapshot;zone:string;quantity:Quantity}) {
  return <div className="array-dynamic"><div className="dynamic-frequency"><span>{Math.max(...data.freq_mhz).toFixed(0)}</span><span>MHz</span><span>{Math.min(...data.freq_mhz).toFixed(0)}</span></div><img src={tile.src} alt={`${quantity} dynamic spectrum`} draggable={false}/><div className="dynamic-time"><span>{clock(data.t0,zone)}</span><span>{clock(data.t1,zone)}</span></div><div className="tile-scale"><i className={`scale-${quantity}`}/><span>{quantity==='phase'?'−π … π rad':`${number(tile.min)} … ${number(tile.max)}${tile.log?' · log':''}`}</span></div></div>;
}
