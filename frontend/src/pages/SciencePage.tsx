...
export default function SciencePage({compare = false}: {compare?:boolean}) {
  ...
  const render=async()=>{setBusy(true);setError("");try {
    let pairs=selected.map(s=>s.split(",").map(Number));
    if(kind==="autos")pairs=Array.from(new Set(pairs.flat())).slice(0,6).map(i=>[i,i]);
    const request:Json={pairs,t0:isoInput(window.t0),t1:isoInput(window.t1),fmin:Number(fmin),fmax:Number(fmax),kind,reference,resolution};
    ...
    setParams({t0:request.t0,t1:request.t1,kind,reference,fmin,fmax,pairs:selected.join(";")},{replace:true});
