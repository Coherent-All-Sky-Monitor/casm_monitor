...
import { Antenna, DynamicSpectrum, LayoutMap, location, QUANTITIES, Quantity, Snapshot, Spectrum } from "../components/vis/ArrayPlots";
...
export default function ArrayVisibilityPage() {
  ...
  const draw=(a:Antenna,expanded=false)=>{
    ...
    return <article className={`antenna-plot ${expanded?'expanded':''}`} key={a.packet_idx} style={expanded||arrangement==='compact'?undefined:{gridColumn:location(a).col+1}}>
      <header><button className="plot-title" onClick={()=>setFocus(a.packet_idx)}>{a.station}<small>ant {a.antenna}</small></button><span>{p.is_auto?'auto':`× ${targetReference?.station??'ref'}`}</span></header>
