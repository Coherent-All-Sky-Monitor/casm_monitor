...
function ArrayContext({catalog,observation}:{catalog:Json;observation:Json|null}) {
  ...
  return <><div className="overview-panel-heading"><h3>Array & calibration</h3><span className="overview-now">Now</span></div>
    <div className="overview-array-counts"><strong>{known?selected.size:'?'}</strong><span>beamforming</span><b>{inputs.length}</b><span>wired</span></div>
    ...
    <p className="overview-caption">Green / BF: beamforming · blue: other wired · × empty{!known?' · membership unknown':''}</p>
    <p className="overview-antenna-detail">{antenna?`${antenna.station} · ant ${antenna.antenna} · ${wiringLabel(antenna as any)}`:'Select an antenna for wiring details.'}</p>
    <div className="overview-panel-footer"><Link to="/vis">Inspect antennas →</Link><Link to="/cal">Calibration →</Link></div>
    <details className="overview-evidence"><summary>Calibration & configuration</summary><div className="overview-calibration"><span>Current ledger calibration</span><strong>{cal?.path?.split('/').pop()??'Unavailable'}</strong></div><p>Green follows the inspected nonzero payload of the latest recorded deployment, across all coherent beams. Visibilities and SNAPs default to this membership; layout intent is separate. Runtime readback is unverified.</p><Evidence value={{layout:catalog.current_layout,calibration:cal,deployment:observation?.deployment}}/></details>
  </>;
}
