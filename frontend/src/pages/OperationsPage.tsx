...
function ArrayContext({catalog,observation}:{catalog:Json;observation:Json|null}) {
  const inputs=catalog.inputs??[],known=observation?.deployment?.inspection_state==='complete';
  const selected=new Set<number>(known?(observation?.layout?.points??[]).filter((p:Json)=>p.deployed===true).map((p:Json)=>p.packet_idx):[]);
  ...
  return <><div className="overview-panel-heading"><h3>Array & calibration</h3><span className="overview-now">Now</span></div>
    <div className="overview-array-counts"><strong>{known?selected.size:'?'}</strong><span>in deployed CB weights</span><b>{inputs.length}</b><span>wired</span></div>
    <div className="overview-map" aria-label="Current station layout; north up, east right"><div className="overview-map-direction"><span>North ↑</span><span>East →</span></div><div className="overview-map-grid"><span/>{[1,2,3,4,5,6].map(c=><span key={c}>E{c}</span>)}{rows.map(row=><div className="overview-map-row" key={row}><span>N{String(row).padStart(2,'0')}</span>{[1,2,3,4,5,6].map(col=>{const a=inputs.find((i:Json)=>location(i as any).row===row&&location(i as any).col===col);return a?<button key={col} className={selected.has(a.packet_idx)?'beamforming':''} aria-pressed={focus===a.packet_idx} aria-label={`${a.station}, antenna ${a.antenna}${selected.has(a.packet_idx)?', beamforming':''}`} title={`${a.station} · ant ${a.antenna} · ${!known?'Membership unknown':selected.has(a.packet_idx)?'In deployed CB weights':'Other wired input'}`} onClick={()=>setFocus(a.packet_idx)}>{a.antenna}{selected.has(a.packet_idx)&&<small>BF</small>}</button>:<span className="overview-map-empty" key={col} title={`N${row}E${col}: no wired antenna`}>×</span>;})}</div>)}</div></div>
    <p className="overview-caption">Green / BF: deployed beamforming · blue: other wired · × no antenna. {known?'Latest recorded weights; runtime readback unverified.':'Beamforming membership unavailable.'}</p>
    <p className="overview-antenna-detail">{antenna?`${antenna.station} · ant ${antenna.antenna} · ${wiringLabel(antenna as any)} · ${!known?'Beamforming unknown':selected.has(antenna.packet_idx)?'In deployed CB weights':'Outside deployed CB weights'}`:'Click an antenna for its station, SNAP / ADC and beamforming membership.'}</p>
    ...
    <details className="overview-evidence"><summary>Configuration details</summary><p>Green follows the inspected nonzero payload of the latest recorded deployment, across all coherent beams. Layout intent and the Visibilities inspection preset are separate.</p><Evidence value={{layout:catalog.current_layout,calibration:cal,deployment:observation?.deployment}}/></details>
  </>;
}
...
export default function OperationsPage() {
  ...
  return <div className="workspace-page overview-page">
    ...
    <div className="overview-main-grid"><section className="overview-panel overview-injections" aria-label="Injection recovery history">{history?<InjectionPanel data={history} zone={zone}/>:historyError?<Notice>{historyError}</Notice>:<p className="overview-loading">Loading injection history…</p>}</section>
      <section className="overview-panel overview-array">{catalog.data?<ArrayContext catalog={catalog.data} observation={live.error||catalog.error?null:data}/>:<Notice>{catalog.error||'Loading array configuration…'}</Notice>}{catalog.error&&catalog.data&&<Notice>Configuration refresh failed; the map may be stale.</Notice>}</section></div>
