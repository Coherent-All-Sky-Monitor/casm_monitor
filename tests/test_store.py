...
from casm_monitor.store import ShardReader, ShardWriter, Store, apply_retention
...
def test_retention_only_expired_and_never_referenced(store: Store, shard_writer: ShardWriter):
    ...
    store.submit_job("noop", {"shard": old_referenced["path"]})  # queued -> referenced
    ...
    for keep in (old_pinned, old_referenced, fresh, untracked):
